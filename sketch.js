...
let introModalContainer = null; // 新增：自我介紹彈窗容器
...
class QuizApp {
    ...
    ShineParticle = class {
        constructor(p, x, y) {
            ...
            // 修正：使用全域 p5.Vector（instance 模式下 p.p5 未定義）
...
function createIntroModal() {
    ...
    // 內容容器
    introModalContainer = document.createElement('div');
    introModalContainer.id = 'intro-modal-container';
    introModalContainer.style.position = 'relative';
    introModalContainer.style.background = '#fff';
    introModalContainer.style.padding = '20px 24px';
    introModalContainer.style.borderRadius = '10px';
    introModalContainer.style.boxShadow = '0 12px 40px rgba(0,0,0,0.35)';
    introModalContainer.style.textAlign = 'center';
    introModalContainer.style.minWidth = '280px';
    introModalContainer.style.maxWidth = '90vw';
    introModalContainer.style.zIndex = '30001';
    introModalContainer.style.display = 'flex';
    introModalContainer.style.flexDirection = 'column';
    introModalContainer.style.alignItems = 'center';
    ...
    img.addEventListener('error', (e) => {
        console.error('Intro image failed to load:', img.src, e);
        img.style.display = 'none';
        const note = document.createElement('div');
        note.textContent = '圖片載入失敗，請確認檔案已放在專案資料夾';
        note.style.color = '#666';
        note.style.fontSize = '14px';
        note.style.marginBottom = '12px';
        introModalContainer.insertBefore(note, introModalContainer.firstChild);
    });

    // 文字：學號、姓名
    const line1 = document.createElement('div');
    line1.textContent = '414730571';
    line1.style.fontSize = '20px';
    line1.style.fontWeight = '700';
    line1.style.marginBottom = '6px';

    const line2 = document.createElement('div');
    line2.textContent = '黃榆秦';
    line2.style.fontSize = '18px';
    line2.style.marginBottom = '14px';
    ...
    // 組合
    introModalContainer.appendChild(img);
    introModalContainer.appendChild(line1);
    introModalContainer.appendChild(line2);
    introModalContainer.appendChild(closeBtn);
    overlay.appendChild(introModalContainer);

    // 點擊遮罩關閉
    overlay.addEventListener('click', (e) => {
        if (e.target === overlay) closeIntroModal();
    });

    document.body.appendChild(overlay);
}

function launchIntroModal() {
    createIntroModal();
    const overlay = document.getElementById('intro-modal-overlay');
    if (overlay) overlay.style.display = 'flex';
    if (introModalContainer) introModalContainer.style.display = 'flex';
    try { noLoop(); } catch (e) {}
    // 除錯輸出
    console.log('launchIntroModal -> img src:', document.querySelector('#intro-modal-container img')?.src);
}

function closeIntroModal() {
    const overlay = document.getElementById('intro-modal-overlay');
    if (overlay) overlay.style.display = 'none';
    if (introModalContainer) introModalContainer.style.display = 'none';
    try { loop(); } catch (e) {}
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const menuItems = [
        ...
        { name: '作品四 自我介紹', url: '#', isAction: true, action: launchIntroModal, submenu: null },
